...
const Log = RootLog.getLogger("Lapis.IndexedDBStore");
...
export default class IndexedDBStore implements Store {
  ...
  createStore(db) {
    ...
    for (i = 0; this.indexes && i < this.indexes.length; i += 1) {
      ...
      Log.debug("created index: %s with key_path: %s", this.indexes[i].id,
  ...
  setUpgradeFunction(from_version: number, to_version: number, funct: Function) {
    if (typeof from_version !== "number" || from_version < 1 || !Number.isInteger(from_version)) {
    ...
    if (this.upgrade_functions[from_version]) {
      throw new Error("upgrade function already set for this store for version: " + from_version);
    }
  ...
  upgrade(from_version: number, to_version: number, db, request) {
    ...
    for (i = from_version; i < to_version; i += 1) {
      if (this.upgrade_functions[i]) {
        Log.debug("calling upgrade function for store: " + this.store_id
          + ", from_version: " + i);
